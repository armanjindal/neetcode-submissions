class Solution:
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        ...
        sorted_strings : list[str] = [str(sorted(original_string)) for original_string in strs]
        result_map : dict[str:int] = {}
        result_list : list[list[str]] = []

        for i in range(len(strs)):
            if sorted_strings[i] not in result_map.keys(): # {"abc" : 0}
                result_list.append([strs[i]])
                result_map[sorted_strings[i]] = len(result_list) - 1
            else:
                result_list[result_map[sorted_strings[i]]].append(strs[i])
